...
class ClusterPageTest extends TestCase
{
    ...
    public function test_story_source_drilldown_links_sources_to_angles(): void
    {
        ...
        $this->withUnencryptedCookies($this->readerCookies())
            ->get($this->pathFor($post))
            ->assertOk()
            ->assertSee('Provenance des sources')
            ->assertSee('Matrice des angles')
            ->assertSee('Drill Left')
            ->assertSee('Drill Center')
            ->assertSee('Drill Right')
            ->assertSee('La source de gauche soutient cet angle')
            ->assertSee('Résumé de provenance')
            ->assertSee('Camp dominant')
            ->assertSee('Origine dominante')
            ->assertSee('grimba-source-drilldown__country', false)
            ->assertSee('grimba-story-distribution__signal', false)
            ->assertSee('Camp majoritaire')
            ->assertSee('Origines éditoriales')
            ->assertSee('grimba-source-drilldown__origin', false)
            ->assertSee('Ouvrir dans le dossier')
            ->assertSee('href="#story-article-' . $ids[0] . '"', false)
            ->assertDontSee('Propriété')
            ->assertDontSee('Lire cette source')
            ->assertDontSee('OpenAI')
            ->assertDontSee('Claude');
    }
