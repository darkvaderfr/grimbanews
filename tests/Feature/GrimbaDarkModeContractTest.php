...
class GrimbaDarkModeContractTest extends TestCase
{
    ...
    public function test_surface_renders_dark_mode_attrs_when_cookie_is_dark(): void
    {
    ...
    public function test_surface_renders_light_mode_attrs_by_default(): void
    {
        foreach (self::readerSurfaces() as $path) {
            ...
            $this->assertStringContainsString('data-bs-theme="light"', $html, "$path must default data-bs-theme to light.");
            $this->assertStringContainsString('data-grimba-theme-pref="light"', $html, "$path must default data-grimba-theme-pref to light.");
        }
    }

    public function test_surface_carries_fouc_guard_script(): void
    {
        foreach (self::readerSurfaces() as $path) {
     ...
     */
    public function test_surface_has_no_hardcoded_white_inline_backgrounds_in_dark(): void
    {
        foreach (self::readerSurfaces() as $path) {
            $html = $this->withUnencryptedCookies(['grimba_theme' => 'dark'])
                ->get($path)
     ...
     */
    public function test_dark_cookie_path_emits_single_body_class(): void
    {
        foreach (self::readerSurfaces() as $path) {
            $html = $this->withUnencryptedCookies(['grimba_theme' => 'dark'])
                ->get($path)
