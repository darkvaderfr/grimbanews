...
| Immigration category — 78 articles (was 0) | 🟡 | 2026-05-17+18: classifier fix moved 46 keyword-matching posts in; `grimba:seed-immigration-sources` + `grimba:seed-thin-category-sources` + RSS poll added 32 more from La Cimade (50), Refugees International (10), GISTI, France TV Info Immigration vertical (#93 sub-feed), Le Monde Afrique vertical. Still short of 500. **MPI** URL corrected to `migrationpolicy.org/rss.xml` and re-enabled. **TNH, France terre d'asile, UNHCR feeds disabled** (HTTP 403/404 on multiple URL probes — operator can re-enable when a working feed URL is found). |
